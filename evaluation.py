...
def calc_key_figures(
    ...
    volatility = volatility.loc[start:end,].bfill() # Note: bfill causes forward looking bias for the first few days but is a reasonable approximation in this case
    ...
    if "adjusted_sharpe" in key_figures:
        sharpe = _calc_sharpe(tot_returns)
        skew = tot_returns.skew()
        kurt = tot_returns.kurt()
        result["adjusted_sharpe"] = sharpe * (1 + sharpe/6 * skew - (sharpe**2)/24 * kurt)
...
def _plot_kf_table(ax1, key_figures):
    kfs = [
        ["Adjusted Sharpe (yearly)", f'{key_figures["adjusted_sharpe"]:.2f}'],
